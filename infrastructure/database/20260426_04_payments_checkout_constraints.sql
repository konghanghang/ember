...
-- 用途：统一收口 payments 的 checkout 并发约束与 stripeSessionId 唯一索引。
--
-- 改了哪些表、字段、索引、约束：
--   1. 为 `(userId, planId) WHERE status='pending'` 增加 partial unique，
--      阻断双标签页连点结账造出多条 pending 订单。
--   2. 删除 baseline 中重复的 stripeSessionId 唯一索引，保留单一 partial unique
--      `idx_payments_stripe_session_id WHERE stripeSessionId <> ''`。
--
-- 是否需要回填：
