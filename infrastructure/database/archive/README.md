...
普通使用者不需要关注本目录。新装库或支持窗口内升级请按 [`infrastructure/database/README.md`](../README.md) 走顶层 baseline 与后续顶层增量。

当前直接升级支持起点是 `2026-06-05` / v1.6.0 截点。旧于该截点且未执行过已归档增量的数据库，启动期 Migrate 不会自动从本目录回放 SQL；需要人工对齐到支持起点 schema，或先升级到包含对应增量的中间版本。
...
- 不要把本目录 SQL 整体搬回 `infrastructure/database/` 顶层补救旧库：多份 baseline 会触发启动期 fail-fast，历史回填 / seed / 兼容脚本也不一定适合当前运行链路。
