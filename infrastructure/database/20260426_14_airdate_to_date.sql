...
--
-- 改了哪些表、字段、索引、约束：
--   - tv_calendar_items.airDate: timestamptz → date
--   - media_gaps.airDate: timestamptz → date
--
...
-- SELECT count(*) FROM tv_calendar_items WHERE EXTRACT(HOUR FROM "airDate" AT TIME ZONE 'UTC') != 0;
-- SELECT count(*) FROM media_gaps WHERE EXTRACT(HOUR FROM "airDate" AT TIME ZONE 'UTC') != 0;

ALTER TABLE tv_calendar_items
  ALTER COLUMN "airDate" TYPE date USING ("airDate" AT TIME ZONE 'UTC')::date;
