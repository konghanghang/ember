-- 00000000_baseline_20260502.sql
--
-- v1.4.0 截点 fresh-install schema baseline（cutoff: 2026-05-02）
--
-- 本文件是 v1.4.0 上线后唯一的"新装库初始化入口"。
-- 内容由 v1.4.0 截点的运行库 pg_dump --schema-only 整理而来：
--   - 去除 pg_dump 通用 SET preamble 与 \restrict/\unrestrict 修饰符
--   - 去除 public. schema 前缀以与历史 baseline 风格一致
--   - 表 / 索引按字典序排列，便于后续 schema diff 验证
--   - 末尾追加 deterministic seed（settings 5 条 + plan_groups.DEFAULT）
--
-- 执行行为：
...
--
-- 真相源优先级：
--   1) 本文件直接对齐运行库 pg_dump 输出（v1.4.0 截点）
...
--
-- 历史合并源（24 份顶层 SQL 拼接式 baseline）整批归档于
-- archive/pre-20260502/，仅供追溯，不再属于现行执行链路。
--
...
CREATE TABLE redemption_codes (
    ...
    registration_plan_group character varying(50)
);
