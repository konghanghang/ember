...
func getConfigDefinitions() []ConfigDefinition {
			...
			Placeholder: `[
  {
    ...
    "sortOrder": 10
		...
		{
			Key:            "registration_allowed_email_domains",
			Group:          ConfigGroupBusiness,
			GroupLabel:     "基础业务",
			Label:          "注册邮箱域名白名单",
			Description:    "每行一个允许注册的邮箱域名（精确匹配，不做后缀匹配）；留空表示不限制",
			Type:           ConfigValueString,
			DefaultValue:   "",
			...
			AllowEmpty:     true,
			EmptyValueMode: ConfigEmptyValueDisable,
			EmptyValueHint: "保存为空值后将不再限制注册邮箱域名。",
			Validate:       validateRegistrationEmailDomains,
			Normalize:      normalizeRegistrationEmailDomains,
		},
		{
		...
		},
		{
			...
			GroupLabel:     "媒体集成",
			Label:          "前端 Emby 地址",
			Description:    "用户在控制台看到并跳转到 Emby 时使用的前端地址；沿用历史键名，不再作为环境变量使用",
			Type:           ConfigValueURL,
			Editable:       true,
		...
		},
		{
			...
			GroupLabel:         "任务调度",
			Label:              "全局业务时区",
			Description:        "统一用于调度、日期边界、排行榜、播放记录和用户可见时间的 IANA 时区",
			Type:               ConfigValueString,
			Editable:           true,
			RestartRequired:    true,
		...
		{
			Key:                "TV_CALENDAR_STARTUP_SYNC_ENABLED",
			EnvKey:             "TV_CALENDAR_STARTUP_SYNC_ENABLED",
			DisableEnvFallback: true,
			Group:              ConfigGroupSchedule,
			GroupLabel:         "任务调度",
			Label:              "追剧日历启动补偿同步开关",
			Description:        "控制 API 启动后是否自动执行一次追剧日历补偿同步",
			Type:               ConfigValueBoolean,
			Editable:           true,
			RestartRequired:    true,
			DefaultValue:       "true",
		...
		},
		{
			...
			GroupLabel:         "部署与密钥",
			Label:              "Admin API Key",
			Description:        "外部自动化调用管理员接口使用的 API Key hash",
			Type:               ConfigValueSecret,
			Editable:           false,
			Sensitive:          true,
			AllowEmpty:         true,
			EmptyValueMode:     ConfigEmptyValueDisable,
			EmptyValueHint:     "未配置时外部 Admin API Key 调用不可用。",
			ReadOnlyHint:       "该项由专用生成和禁用接口管理，后台不会展示明文或允许手填 hash。",
