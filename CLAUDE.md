...
- **禁止**：直接启动/停止/重启开发服务器（`npm run dev`、`npm start` 等）
- **应该**：修复代码问题，确保编译通过，然后告诉用户如何验证
- **原因**：开发服务的管理应由用户控制，AI 只负责代码质量
...
```text
✅ 正确：
"代码已修复并通过编译。请执行以下步骤验证：
1. 重启开发服务器：npm run dev
2. 访问 http://localhost:3000/login
3. 使用 admin/admin123 登录"

❌ 错误：
直接执行 npm run dev 启动服务
```
