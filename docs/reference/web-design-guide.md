...
- `EmberPageHeaderCard`
  ...
  - 中心页必须由容器统一承载唯一页头和分段导航；嵌入子页使用 `hideTitle` 收起重复标题与说明，但统计 badge、操作区和主体内容必须保留。
...
- 筛选按钮组统一右对齐：
  ...
  - 移动端换行后也保持按钮组靠右
  - 顺序固定：左 `重置`，右 `查询`
- 若存在“查询”，筛选操作区默认不再放“刷新”（避免重复语义）。
- 多数据区页面如果“查询”只筛选其中一张列表，而统计、日志或辅助面板会独立变化，可以在 `Header Card` actions 提供页面级“刷新”：必须一次刷新全部数据区、保留当前筛选，并提供 loading/disabled 反馈；禁止把该动作塞回筛选按钮组。
...
- 标题与筛选并排且造成大面积留白
- 单筛选项强行使用多列布局导致画面松散
- 同一筛选区同时出现“查询 + 刷新”重复动作；多数据区的页面级全量刷新必须放在页头 actions 并与筛选语义分离
