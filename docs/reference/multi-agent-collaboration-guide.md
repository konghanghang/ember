...
- `.codex/agents/` 中的角色默认不设置 `model` 和 `model_reasoning_effort`；职责、权限和输出要求由角色文件定义。
- 未显式指定启动参数、也未设置 `[agents]` 对应默认值时，subagent 继承父代理的模型和推理强度。
- 需要调整时，优先在个人 `config.toml` 的 `[agents]` 中统一设置 `default_subagent_model` / `default_subagent_reasoning_effort`，或按具体任务在启动时显式指定；启动参数优先于这些默认值。
- 如果启动参数或 `[agents]` 默认值指定了模型，但两者都未指定推理强度，则使用该模型的默认推理强度。
- 边界清楚、局部实现型任务，可以按需选择更轻模型；高风险、高耦合、强推理任务，可以按需选择更强模型和其支持的推理强度。
- 只有存在明确的成本、延迟或评测依据时，才考虑在角色文件中固定模型或推理强度，并记录理由；角色文件中的固定值优先于启动参数和 `[agents]` 默认值。
