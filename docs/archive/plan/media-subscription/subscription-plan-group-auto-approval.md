...
- 已实现 `PlanGroup.subscriptionAutoApproveDailyLimit`、`subscriptions.review_source`、用户账号可提交校验与按分组日额度自动通过。
- 已实现自动通过后的用户结果通知、管理员只读通知、后台审批来源展示，以及重提交流程的同口径处理。
- 已补 API / Bot / Web 回归测试和集成测试，覆盖额度命中、超额回退人工审核、审批来源展示与通知链路。
- 已完成 `go test ./internal/services/subscription ./internal/app`、`go vet ./...`、`go build ./...`、Bot/Web 对应测试，以及全仓 `scripts/test/all.sh` 验证。
- 稳定结论已同步到 `docs/system-architecture.md`、`docs/reference/data-model-reference.md`、`docs/reference/api-endpoint-catalog.md`，本稿现在只保留历史追溯价值。
...
- 稳定结论已同步到 `docs/system-architecture.md`
  ...
  - 自动通过与人工审批的通知边界
- 字段与枚举已同步到 `docs/reference/data-model-reference.md`
- 创建订阅成功响应中的 `status / autoApproved / subscriptionId` 已同步到 `docs/reference/api-endpoint-catalog.md`
