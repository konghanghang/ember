...
- 新增能力：
  ...
  - 聚合视图顶部收口为“页头卡 + 剧卡列表”两层结构；全局统计压缩进页头内的紧凑 badge，排序控件并入页头底部，不再额外叠加摘要卡和独立总览卡。
- 首版范围：
...
- 前端约束：
  ...
  - 聚合视图的总览信息默认收进 `EmberPageHeaderCard` 内部，不再额外叠加第二层摘要卡或独立总览卡；顶部重点只保留标题、操作、筛选、紧凑统计和排序控件。
