...
- ✅ 部署环境盘点、baseline 精简归档和交叉引用收口已完成
- ✅ `docs/plan/README`、`docs/proposals/README`、`docs/proposals/plan-inventory.md` 已移除本方案的现行入口占位
- ✅ 稳定结论已以下沉到 `docs/system-architecture.md`、`infrastructure/database/README.md` 与相关 runbook 为准

## 归档判断

- 当前已完成归档。
- 原因：代码与部署基线已经稳定，现行规则已有稳定落点；本文只保留方案背景、整改范围与决策溯源价值。
...
- 当前盘点入口：
  - [docs/proposals/plan-inventory.md](</Users/konghang/data/github/ember/docs/proposals/plan-inventory.md>) 已把本方案更新为“已归档”

## 退场说明

- 本文档已退出 `docs/plan/`，不再承担“现行部署规则”说明职责；现行基线以 `docs/system-architecture.md`、`infrastructure/database/README.md` 与 `docs/runbooks/` 为准。
- 归档完成后，本文只保留以下价值：
  - 为什么当时要把 `AutoMigrate`、`VerifySchema`、`initdb/`、镜像与 compose 基线一起收口
  - 哪些 schema / 部署风险被明确判定为当前边界的一部分
  - 哪些被项目级规则否决，例如“不引入数据库 FK / 级联策略”
