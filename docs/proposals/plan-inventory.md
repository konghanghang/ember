...
- 其中 `3` 份更适合进入“归档准备”而不是继续作为核心实施稿：计划 3、计划 6、计划 8
- 其中 `4` 份属于“主干完成，保留尾项”：计划 1、2、4、7
- 其中 `1` 份仍应明确视为“继续进行中”：计划 5
- 计划 3 和计划 4 已补齐稳定结论、交叉引用与退场说明；`pickTargetSeasonNumbers` 已落地，计划 3 已收敛到归档准备，计划 4 仍保留独立实现尾项
...
| 4 | `media-subscription/tv-calendar-and-tmdb-key-protection.md` | 主干完成，保留尾项 | TMDB / MoviePilot / Stripe / SMTP 上游错误脱敏、`httpx.InternalError`、webhook `tmdbId` 命中精度、`tmdb_cache` GC、`resolveSeriesTMDBIDBySeriesID` 5 分钟缓存、同 key TMDB in-flight 去重、当前周纠偏落库已完成；稳定结论与退场条件已补齐 | TMDB 三层缓存更深层统一 / 观察性尾项 | 继续保留在 `docs/plan/` |
