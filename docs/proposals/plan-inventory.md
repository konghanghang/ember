...
|------|------|----------|------------|------------|----------|
| 1 | `access-auth/auth-and-account-integrity-hardening.md` | 主干完成，保留尾项 | 验证码发送限流并发收口、注册/重置密码验证码事务路径、注册回滚补偿、统一错误响应、`CheckExpiredUsers` cancel/失败上限均已落地 | ConfigService 敏感回显、DI 治理 | 继续保留在 `docs/plan/` |
