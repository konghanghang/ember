...
4. 电影榜：
   ...
   - 过滤后重新排序并截断前 `10`。
5. 剧集榜：
   - 先拉取当前时间窗口内全部 episode 聚合结果；
   - 按允许 episode `ItemId` 过滤；
   - 再回查条目详情按 `SeriesId` 聚并；
   - 聚并后重新排序并截断前 `10`。
6. 总播放时长：
   - 改为统计过滤后实际参与榜单计算的电影 + episode 总时长；
   - 不再使用当前“全量播放记录总时长”口径。
7. 正式榜单入库和通知继续复用现有流程，只是输入结果改为 allowlist 过滤后的数据。
...
- 将“排行榜支持媒体库 allowlist，空配置视为全库统计”的稳定结论提炼到 `docs/system-architecture.md`
- 将排行榜页的管理员配置区块职责同步到 `docs/reference/web-information-architecture.md`
- 若第一版验证后确认性能可接受，再将“不新增本地持久化归库缓存”的结论保留为现状说明；若性能不足，再单独起下一份优化方案
- 本方案在功能落地、验证完成、现行文档同步后移入 `docs/archive/plan/media-subscription/`
