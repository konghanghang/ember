...
优先复用现有字段与状态；如实现需要 schema 变化，必须同步幂等 migration。前端实现必须遵守 Ember 风格，设计与交互基线以 [设计规范](../../reference/web-design-guide.md) 为准，无视觉规范特例。列表响应继续使用 data，字段保持 camelCase。
...
逐项同步 system-architecture 与相关 reference。完成验证后记录结果和边界，迁入 docs/archive/plan/media-subscription/，更新索引和盘点；未推送不代表未完成本轮本地交付。
