...
当前后端接口已经支持 `clear=true`，但前端没有提供独立入口。

这会导致一个问题：

1. 管理员能覆盖敏感值
2. 但不能直接把数据库覆盖值清掉并回退到 env / default

这不是小问题。没有“清空”操作，配置生命周期就是残缺的。
...
1. 能覆盖 `database > env > default > unset`
2. 能覆盖敏感值不回显
3. 能覆盖 `clear=true`
4. 能覆盖媒体/邮件组测试结果展示
...
1. 每个可编辑敏感项提供独立“清空数据库覆盖值”动作
2. 非敏感但允许空值的项，也要区分“设为空值”和“回退来源”
