...
## 2. 读取优先级固定为：数据库覆盖值 > 环境变量 > 默认值

这是整个方案的兼容基础。

含义：

1. 老部署什么都不改，系统仍按 env 行为工作
2. 某项配置一旦在设置中心保存，数据库值立即接管
3. 未配置 env、也未配置数据库的项，才回退到代码默认值
...
- settings 无值且 env 有值 -> 用 EMBY_URL
- 两者都没有 -> 视为未配置
```
...
2. 自动化测试仍未完全覆盖到 handler 与真实组件挂载层：
   - 已补 `ConfigService` 解析优先级/敏感值错误路径测试
   - 已补设置中心前端草稿与清空交互的纯逻辑测试
   - 仍缺少分组测试接口的 handler 级测试
   - 仍缺少真实组件挂载级别的页面交互测试

3. 尚未在真实数据库执行迁移；生产环境需要手工执行：
...
2. 补分组测试接口的 handler 测试和真实组件交互测试
3. 补剩余边界项治理：优先梳理 `STRIPE_WEBHOOK_SECRET`、`TELEGRAM_BOT_TOKEN`、`TELEGRAM_WEBHOOK_SECRET`、`WEBHOOK_URL`
...
所以后续工作重点不再是“要不要做设置中心”，而是：

1. 把剩余配置继续迁进统一配置层
2. 把缺少的测试、清空语义、上线流程补完整
3. 避免新旧接口、新旧读取路径再次分叉

### 2. 仍然存在的设计缺口

#### A. 敏感项已支持显式清空，但仍缺少更完整的组件级回归测试
...
4. 后续补齐媒体/邮件组测试结果展示与 handler 级验证
...
1. 配置修改日志至少记录 `key`、操作人、时间、动作类型
2. 记录“更新”“清空”“导入 env”三类动作
