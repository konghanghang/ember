...
- 非空值必须是规范化绝对目录，不能是 `/`，也不能包含 NUL、`.` 或 `..` 等歧义段。
...
2. 将 `/` 分隔的相对路径转换为当前 Linux 容器文件路径，并拼接 `PLAYBACK_LOCAL_MEDIA_ROOT`。
3. 再次确认结果仍位于配置根目录内，拒绝绝对路径、父目录穿越和根目录逃逸。
4. 只尝试打开这一条精确路径，不执行目录遍历、glob、大小写折叠或同名搜索。
5. 文件存在、是普通文件且可读时返回已打开的只读文件；不存在、目录、权限不足或打开失败时返回类型化 miss/unavailable，由 Gateway 继续 Emby fallback。
...
- 本地解析器测试：精确文件命中、不存在、目录、不可读、根目录未配置、根目录不可用和路径逃逸；测试不得加入 SHA1、大小或修改时间匹配条件。
