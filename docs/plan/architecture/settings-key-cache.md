...
- 将“ConfigService 对 `settings` 使用按 key 的进程内 TTL 缓存，TTL=60s，写后失效”的稳定结论提炼到 `docs/system-architecture.md`
- 若后续确认仍需更强缓存能力（批量快照、跨实例同步、外部缓存），另起新方案，不在本稿扩展
- 本方案在功能落地、验证完成、现行文档同步后移入 `docs/archive/plan/architecture/`
