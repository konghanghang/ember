...
- 管理员端和用户端都保留推荐快捷范围：`7d / 30d / 90d`
- 管理员端和用户端都支持自定义日期范围选择
- 自定义日期范围最多允许选择 `3` 个月，前端限制选择范围，后端兜底校验
...
- 两个接口统一支持：
  - `range=7d|30d|90d|all`，默认 `30d`
  - `startDate`
  - `endDate`
- 当传入 `startDate/endDate` 时，优先按自定义日期范围查询
- 自定义日期范围最大跨度限制为 `92` 天
- 响应字段统一使用 `data`，包含：
  - `userId`
  - `username`
...
1. 前端通过本地 `userId` 请求用户画像接口，并可带快捷时间窗口或自定义日期范围。
2. 后端读取本地 `users` 表，校验用户存在并映射 `embyId`。
3. 服务优先解析 `startDate/endDate`，否则回退到 `range`，统一换算为查询窗口。
4. 服务校验自定义日期范围最大跨度不超过 `92` 天。
5. 服务基于最终查询窗口拼接 `PlaybackActivity` 查询条件，分别计算摘要、分布和最近记录。
6. 服务构建勋章规则结果，统一输出格式化后的画像响应。
7. 管理员端渲染单用户画像页面，并提供跳转到播放历史的联动入口。
8. 用户端渲染“我的画像”页面，仅展示自己的摘要、活跃时段、设备偏好、画像标签和最近播放记录。
...
- 自定义日期范围缺少开始或结束日期：返回 `400`
- 自定义日期范围超过 `92` 天：返回 `400`
