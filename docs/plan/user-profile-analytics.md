...
- 现有限制：管理员侧画像已落地，但普通用户还没有自己的画像入口。
...
- 普通用户新增“我的画像”页面，路由为 `/console/profile-analytics`。
- 用户侧页面默认只展示自己的画像数据，不提供管理员视角跳转或审计入口。
...
- 用户侧前端直接调用 `GET /api/v1/profile/analytics`，不新增单独的数据结构。
...
5. 管理员端渲染单用户画像页面，并提供跳转到播放历史的联动入口。
6. 用户端渲染“我的画像”页面，仅展示自己的摘要、活跃时段、设备偏好、画像标签和最近播放记录。
...
- Web：有，新增管理员用户画像页、用户端“我的画像”页、路由、API 定义、用户管理入口和播放历史跳转。
- Bot：无。
- 配置/部署：无。
...
- 普通用户可从控制台进入“我的画像”，默认看到自己的 `30d` 数据。
- 普通用户切换时间窗口后，摘要、活跃时段、标签和最近播放记录同步变化。
