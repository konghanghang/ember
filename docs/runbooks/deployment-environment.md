...
`ember-bot` 通过 `profiles: ["bot"]` 控制；`ember-gateway` 和内置 `redis` 通过 `profiles: ["gateway"]` 控制，默认不启动。启用 Gateway 时使用包含统一入口的新镜像并执行 `docker compose --profile gateway up -d`，Compose 会等待 Redis healthy 后再启动 Gateway。
