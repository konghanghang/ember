...
- DirectPlay 使用两层同设备进程缓存，均不按 PlaySessionId 隔离：媒体解析缓存最多 1024 条、按最早到期淘汰，绑定用户/登录/设备/媒体路径/UA、source 与 playback 配置版本/凭证/Provider 参数及目录；命中跳过源解析与目标查重并引用仍有效的最终直链。媒体截止于文件解析开始后 30 秒与引用 URL 缓存原期限的较早值，外部同路径替换/删除最多延迟此窗口后发现。直链层仍为 1024 条 LRU、单 URL 最大 16 KiB，截止于取链完成后 30 秒与链接到期前 10 秒的较早值。两层命中都不续期，不落 Redis/数据库。每次仍执行实时身份/证明、账号/套餐、独立 Redis 准入和最终确认；Stopped 释放租约但保留缓存。任一账号恢复探测绕过媒体缓存，playback 探测及新转存实际取链。同键昂贵查询跨 session 串行、获锁后加载账号；错误/取消/最终确认失败不发布媒体缓存。命中不共享任务或用量，继续访问采样，不回写虚构的健康成功。决策日志以 mediaResolutionCache/downloadURLCache=hit|miss|bypass 区分；fake 已覆盖跨会话 Provider 调用减少，真实客户端延迟收益尚未验收。
