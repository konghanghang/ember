...
当前已落地 115 Cookie 模式的账号控制面、只读凭证验证、上传信息、旧 SHA1 查重和秒传初始化状态适配器；尚未实现目标目录复核、下载直链或播放网关，也未调用真实 115 账号：
...
- `integrations/p115.Provider`：继续定义上传信息、SHA1 搜索、秒传初始化、目标复核、下载地址和串行删除等 Provider-neutral 语义；目标复核、下载地址和删除尚无网络实现
