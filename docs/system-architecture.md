...
- DirectPlay 最终下载地址使用 Gateway 进程内同设备 LRU（最多 1024 条、单 URL 最大 16 KiB），按用户/登录映射/设备、账号配置与凭证、目标文件和真实 UA 隔离，不含 `PlaySessionId`；退出重播可在最多 30 秒且早于直链到期 10 秒的窗口内复用，命中不续期。源解析、目标查重、实时账号与 Redis 准入/最终确认仍执行；Stopped 只释放租约，冷却探测与新转存必须实际取链。缓存不进入 Redis/数据库，不共享任务或用量，不用缓存命中回写 playback 健康成功；源解析成功仍正常采样。单条决策日志以 `downloadURLCache=hit|miss|bypass` 区分结果，真实客户端复用和性能收益未验证。
