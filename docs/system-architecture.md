...
- PlaybackInfo 的 Debug 意图诊断只记录固定版本合同中的白名单字段状态，区分客户端请求、网关补查及证明复用；进程内脱敏 `itemRef` 关联同一登录/设备/条目的请求与视频，`sessionRef` 关联播放事件。诊断不记录原始请求体或会话、不改变透明代理与证明资格、不参与路由或缓存判定；详情探测与正式播放的稳定差异仍需客户端对照取证。
