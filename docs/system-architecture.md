...
- `ResolvePlaybackRoute(ctx, userId, now)`：在同一 PostgreSQL 事务内读取管理员或普通用户的有效套餐、Emby Policy 模板和精确 personal/shared playback 非敏感元数据；两种角色均按套餐选择账号来源与转存配额，管理员不隐式改走共享账号或绕过准入，个人账号管理仍限普通用户。Redis 准入成功后 `AcquirePlaybackRoute` 才按 account ID/owner/`config_version` 锁定并解密凭证
- PlaybackInfo 的 Debug 意图诊断只记录固定版本合同中的白名单字段状态，区分客户端请求、网关补查及证明复用；进程内脱敏 `itemRef` 关联同一登录/设备/条目的请求与视频，`sessionRef` 关联播放事件。诊断不记录原始请求体或会话、不改变透明代理与普通证明资格。独立的新增转存策略只在客户端 POST 明确 `IsPlayback=true` 且 Emby 成功响应匹配时，为选定媒体源记录固定 30 秒许可；复用现有 proof 缓存，不增加 SQL 或第二套缓存。许可绑定身份、条目、source、session 和 proof 代次；Gateway GET 与缓存命中不授予或续期，成功 Stopped 撤销且不依赖 Redis 租约存在。无许可仍可复用已有目标/十分钟跨会话缓存；目标缺失则在内容锁内、配额和任务之前以 `playback_intent_required` 回退，HEAD 不新增转存。该许可控制开始新转存时的准入，不能回滚已开始的操作；其他播放器缺少明确起播字段时首次目标缺失走 Emby，客户端兼容证据不能由单元测试代替。
- 内部 PlaybackInfo GET 与客户端响应观察共用 proof 缓存内有界的请求期令牌；缓存复查/登记和终态检查/整 item 替换均在同一锁内。客户端响应入口废弃同 item 的旧 proof/两类旧令牌，终态只有自身令牌有效才替换；内部发布只废弃其他内部结果，不撤销读取 body 中的客户端观察。旧 GET 不能覆盖新许可或恢复已失效证明，旧客户端失败收尾也不能清除较新快照；过时内部响应丢弃，不借用新许可。实际观察退出后释放令牌，不新增 SQL、持久 tombstone 或第二套媒体缓存。
...
- source 目录读取使用 Adapter 内的在途请求合并：按账号/精确凭证/Provider UA/AppType/root/相对父目录摘要隔离，共享父目录解析与完整分页，各文件独立匹配；完成即移除，不引入 TTL 缓存。等待者独立取消，全部离开取消上游；共享读取总预算 30 秒，耗尽按 Provider unavailable 处理，单 HTTP 仍为 10 秒。账号/用户门控、Redis 配额和下载 URL 不共享。Debug `source_directory_resolved` 仅记录合并人数、条目数、耗时与结果；中间目录重名选择仍未证实，不能把最终文件唯一性表述为完整路径唯一性
...
- DirectPlay 每次入口返回仅用于日志的 Timing 快照，成功/失败均附在 Gateway 单条最终决策日志：directPlayMs、prepare/sourceResolve/targetSearch/lockWait/preID/rapidUpload/challenge/targetVerify/transferCommit/downloadURL 的 Ms 与 Calls，以及未分类 otherMs；重复调用累计，未执行阶段省略。使用单调时钟，不改业务时区或控制流。源目录共享任务 Debug 另含 parentResolveMs/listMs/listPages；统计解释见端到端流程第 8.3 节，Gateway durationMs 不等于首帧耗时
...
- 精确 AuthenticateByName 请求直接交给 Emby 判定；应用头/query 元数据只做可选有界审计采集，缺失、冲突、未知字段或旧 Token 不再触发本地登录 `401`。元数据采集失败仅输出固定 Debug 原因，不改写请求。
- 认证响应透明转发；上游 `200` 最多旁路检查 `1 MiB`，按 Content-Type 解码 JSON/XML `AuthenticationResult`，以 `User.Id/AccessToken/ServerId` 建立映射。XML 要求单一完整根、不解析外部实体；未知字段不重编码，状态/Header/原始字节保持。
- public users/无 Index 公开用户头像不要求本地应用元数据，无 Token 时由 Emby 决定响应；携 Token 时保留映射、撤销和用户资格检查。其余用户接口、普通 API、视频及 WebSocket 不继承公开权限。
...
- 认证成功响应的 `SessionInfo.DeviceId/Client` 优先作为非权威设备/客户端审计信息；缺失或不合法时回退到有界请求元数据，无可用值允许为空。二者不能替代 `User.Id/ServerId/AccessToken` 身份绑定。
- 其他请求按 [Emby Gateway 客户端兼容矩阵](./reference/emby-client-compatibility-matrix.md) 收集直接 Token Header、应用头 Token 和固定 query aliases；非空候选同值才调用 `ResolvePrincipal`。应用元数据缺失/扩展不影响身份解析；重复身份来源、空直接 Token、冲突或非法 grammar 仍返回 `401`，用户不可用/到期返回 `403`，取消/deadline 返回 `499/504`，身份存储故障返回 `503`。
...
- 三类固定 `POST /Sessions/Playing*` 请求只在本地身份门控成功后最多旁路读取 `64 KiB` JSON 并恢复原始 body，提取有界 `ItemId/MediaSourceId/PlaySessionId/PositionTicks/IsPaused` 用于排障及成功转发后的既有租约更新，Stopped 同时撤销已记录的匹配新增转存许可；未通过身份门控时不读取 body，非法、超大或不支持编码的已认证 body 仍原样透明转发，只记录固定 `snapshotState`。开始/停止成功、任一会话失败和失败后的首次恢复使用中文 Info 事件；正常 Progress 心跳只在 Debug 输出，避免每 10 秒刷 Info
...
- bootstrap allowlist 只覆盖层级精确的 SystemInfoPublic、public 用户列表、无 Index 用户头像和已确认 Branding 路径。用户列表/头像无 Token 时不要求应用元数据；Branding Configuration 仍要求严格 query 元数据并受 Web 开关控制。发现、Quick Connect、其他 Branding 和猜测路径继续受 Token 门控。
...
- DirectPlay 使用两层同设备进程缓存，均不按 PlaySessionId 隔离：媒体解析缓存最多 1024 条、按最早到期淘汰，绑定用户/登录/设备/媒体路径/UA、source 与 playback 配置版本/凭证/Provider 参数及目录；命中跳过源解析与目标查重并引用仍有效的最终直链。媒体截止于文件解析开始后 10 分钟与引用 URL 缓存原期限的较早值，外部同路径替换/删除最多延迟此窗口后发现。直链层仍为 1024 条 LRU、单 URL 最大 16 KiB，截止于取链完成后 10 分钟与链接到期前 10 秒的较早值。两层命中都不续期，不落 Redis/数据库。每次仍执行实时身份/证明、账号/套餐、独立 Redis 准入和最终确认；Stopped 释放租约但保留缓存。任一账号恢复探测绕过媒体缓存，playback 探测及新转存实际取链。同键昂贵查询跨 session 串行、获锁后加载账号；错误/取消/最终确认失败不发布媒体缓存。命中不共享任务或用量，继续访问采样，不回写虚构的健康成功。决策日志以 mediaResolutionCache/downloadURLCache=hit|miss|bypass 区分；fake 已覆盖跨会话 Provider 调用减少，真实客户端延迟收益尚未验收。
...
- 缺少起播许可的 `playback_intent_required` 仍使用 `code=direct_play_fallback`，但记录 `directPlayResult=skipped` 和“无起播许可，跳过新增转存”，不冒充 115 Provider 故障；实际 Emby 响应的状态、fallbackResult 和 info/warn 保留。只有 STRM 路径但 Emby 没有实际媒体挂载的部署，回退仍可能返回 404，不构成可用视频源兜底。
- Debug 追加视频入口的规范化 Range/预取分类、按需 PlaybackInfo 开始/结束、DirectPlay 固定阶段与排队开始/结束、缓存结果、租约新建/复用、播放事件租约更新及 Emby fallback 开始/响应头/代理退出耗时。进程生成 requestId 贯穿单请求，带随机种子的 sessionRef 关联视频与播放事件，不输出原始会话或复用 Redis Key；finished/completed 只表示阶段返回，不能当作播放成功。Info 仍只有唯一视频最终决策，新增过程日志不改变授权、缓存、租约和代理行为，详见播放链路参考的日志字段说明。
...
- 同步数据库操作与锁等待使用从调用方 context 派生的两分钟预算；worker 的取消贯穿到同步入口。已发出的 Emby HTTP 仍受原客户端 timeout 管理，取消不代表远端未写入。解锁使用独立五秒窗口，panic 同样先清理，未知锁状态连接丢弃；worker 取消后的失败记账也有独立五秒窗口，worker 在每次同步前逐条领取任务，每轮最多 20 条，取消时未开始的任务保持 pending，下一轮可立即领取；进程中断或当前任务记账失败留下的 processing 仍由既有超时回收处理。
